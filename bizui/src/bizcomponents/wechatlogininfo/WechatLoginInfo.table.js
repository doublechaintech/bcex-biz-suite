...
import styles from './WechatLoginInfo.table.less'
...
class WechatLoginInfoTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = WechatLoginInfoBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/wechatLoginInfo/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default WechatLoginInfoTable
