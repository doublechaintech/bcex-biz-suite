...
import styles from './FaultAnswer.table.less'
...
class FaultAnswerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = FaultAnswerBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/faultAnswer/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default FaultAnswerTable
