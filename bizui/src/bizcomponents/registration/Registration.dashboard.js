...
const imageList =(registration)=>{return [
	   {"title":'头像',"imageLocation":registration.avatar},
]}
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/registration/${targetComponent.props.registration.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/registration/${targetComponent.props.registration.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
<Description term="变更请求">{registration.changeRequest==null?appLocaleName(userContext,"NotAssigned"):`${registration.changeRequest.displayName}(${registration.changeRequest.id})`}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"变更请求","changeRequest",RegistrationService.requestCandidateChangeRequest,
	      RegistrationService.transferToAnotherChangeRequest,"anotherChangeRequestId",registration.changeRequest?registration.changeRequest.id:"")} 
...
class RegistrationDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:"登记",cardsFor: "registration",
