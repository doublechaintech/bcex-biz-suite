...
class RegisterationTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/registeration/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
