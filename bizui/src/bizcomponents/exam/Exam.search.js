...
import styles from './Exam.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  const { ExamStatusAssociateForm } = GlobalComponents
  ...
  <div>
	...
	data={{examList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'status')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'status')}/> <WechatUserAssociateForm 
...
class ExamSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {ExamTable} = GlobalComponents;
    const {ExamSearchForm} = GlobalComponents;
    const {ExamModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        ...
        {showDeletionDialog(this,ExamModalTable,"examIds")}
...
export default Form.create()(ExamSearch)
