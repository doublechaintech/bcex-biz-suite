...
import styles from './ChangeRequestType.table.less'
...
class ChangeRequestTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ChangeRequestTypeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/changeRequestType/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ChangeRequestTypeTable
