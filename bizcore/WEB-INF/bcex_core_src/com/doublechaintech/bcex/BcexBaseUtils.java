...
public class BcexBaseUtils {
	...
	public static String getCacheAccessKey(BcexUserContext ctx) {
	...
	protected static BaseEntity loadCanCacheInLocal(BcexUserContext userContext, String type, String id) throws Exception {
	...
	public static <T extends BaseEntity> List<T> collectReferencedObjectWithType(BcexUserContext userContext,
	...
	private static <T extends BaseEntity> void collectReferedObjectIdSet(BcexUserContext userContext,
	...
	public static int getAppBuildVersion(BcexUserContext ctx) {
		return getBuildVersion(getRequestAppVersion(ctx));
	}
	protected static boolean startFromVersion(BcexUserContext ctx, int version) {
     ...
     * "x-app-version" : "9"
	 */
	public static String getRequestAppVersion(BcexUserContext userContext) {
		return userContext.getRequestHeader("x-app-version");
	}
	public static String getRequestAppDevice(BcexUserContext userContext) {
		return userContext.getRequestHeader("x-app-device");
	}
	public static String getRequestAppType(BcexUserContext userContext) {
	...
	public static ChangeRequestType getChangeRequestType(BcexUserContext userContext, String code) throws Exception {
	...
	public static ExamStatus getExamStatus(BcexUserContext userContext, String code) throws Exception {
		String key = "enum:" + ExamStatus.INTERNAL_TYPE + ":" + code;
		ExamStatus data = (ExamStatus) userContext.getFromContextLocalStorage(key);
		if (data != null) {
			return data;
		}
		data = userContext.getDAOGroup().getExamStatusDAO().loadByCode(code, emptyOptions);
		userContext.putIntoContextLocalStorage(key, data);
		return data;
	}

	public static <T> T loadBaseEntityById(BcexUserContext ctx, String type, String id) throws Exception {
		return loadBaseEntityById(ctx, type, id, null);
	}
	public static <T> T loadBaseEntityById(BcexUserContext ctx, String type, String id, Consumer<T> enhancer) throws Exception {
		String key = type+":"+id;
	...
	public static <T extends BaseEntity> void appendLinkToUrl(BcexUserContext ctx, List<T> list,
			Function<T, String> makeFunc) {
		if (list == null || list.isEmpty()) {
			return;
		}
		list.forEach(it -> {
			it.addItemToValueMap(BcexBaseConstants.X_LINK_TO_URL, makeFunc.apply(it));
		});
	}

	public static <T extends BaseEntity> void appendLinkToUrl(BcexUserContext ctx, T obj, String url) {
		if (obj == null) {
			return;
		}
		obj.addItemToValueMap(BcexBaseConstants.X_LINK_TO_URL, url);
	}

	public static ButtonViewComponent addAction(BcexUserContext ctx, BaseEntity obj, String title, String code,
			String linkToUrl) {
	...
	public static void setAction(BcexUserContext ctx, BaseEntity obj, String title, String code, String linkToUrl) {
		ButtonViewComponent actionBtn = new ButtonViewComponent(title, null, code);
		actionBtn.setLinkToUrl(linkToUrl);
