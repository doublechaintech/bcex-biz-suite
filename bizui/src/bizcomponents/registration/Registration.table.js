...
import styles from './Registration.table.less'
...
class RegistrationTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = RegistrationBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/registration/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
