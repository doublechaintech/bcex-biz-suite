...
<c:if test="${ empty examRankingList}" >
	<div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		 ${userContext.localeMap['@not_found']}${userContext.localeMap['exam_ranking']}! 
		 <a href="./${ownerBeanName}Manager/addExamRanking/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
 ...
 <c:if test="${not empty examRankingList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("examRankingList"); 
...
%>
    ...
    <div class="row" style="font-size: 30px;">
		<div class="col-xs-12 col-md-12" style="padding-left:20px">
		<i class='fa fa-table'></i> ${userContext.localeMap['exam_ranking']}(${totalCount})
		<a href="./${ownerBeanName}Manager/addExamRanking/${result.id}/"><i class="fa fa-plus-square" aria-hidden="true"></i></a>
		 
		 		 	<div class="btn-group" role="group" aria-label="Button group with nested dropdown">		
	<c:forEach var="action" items="${result.actionList}">
		<c:if test="${'examRankingList' eq action.actionGroup}">
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${examRankingListName};${examRankingListName}CurrentPage=${page.pageNumber};${examRankingListName}RowsPerPage=${rowsPerPage}/' 
	...
	<table class="table table-striped" pageToken='examRankingListCurrentPage=${currentPageNumber}'>
		<thead><tr>
		<c:if test="${param.referName ne 'id'}">
	<th>${userContext.localeMap['exam_ranking.id']}</th>
</c:if>
...
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateExamRanking/${result.id}/${item.id}/'>${item.name}</td>
	...
	<td class="select_candidate_td"
			...
			data-switch-method="./examRankingManager/transferToAnotherPlatform/${item.id}/"
		...
		<div class="candidate_span" style="display:none;">
			<input type="text" data-provide="typeahead" class="input-sm form-control candidate-filter-input" autocomplete="off" />
				...
				<a href='#${ownerBeanName}Manager/removeExamRanking/${result.id}/${item.id}/' class='delete-action btn btn-danger btn-xs'><i class="fa fa-trash-o fa-lg"></i> ${userContext.localeMap['@delete']}</a>
				<a href='#${ownerBeanName}Manager/copyExamRankingFrom/${result.id}/${item.id}/' class='copy-action btn btn-success btn-xs'><i class="fa fa-files-o fa-lg"></i> ${userContext.localeMap['@copy']} </a>
