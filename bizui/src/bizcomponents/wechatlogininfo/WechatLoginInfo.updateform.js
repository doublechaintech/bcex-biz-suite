...
import styles from './WechatLoginInfo.updateform.less'
...
class WechatLoginInfoUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const wechatLoginInfoId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, wechatLoginInfoId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateWechatLoginInfo`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextWechatLoginInfoUpdateRow`,
        payload: {
          id: owner.id,
          type: 'wechatLoginInfo',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}微信登录信息: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.appId} {...formItemLayout}>
                  {getFieldDecorator('appId', {
                    ...
                    <Input size="large" placeholder="应用程序Id" />
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.openId} {...formItemLayout}>
                  {getFieldDecorator('openId', {
                    initialValue: selectedRow.openId,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <Input size="large" placeholder="开放Id" />
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.sessionKey} {...formItemLayout}>
                  {getFieldDecorator('sessionKey', {
                    initialValue: selectedRow.sessionKey,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <Input size="large" placeholder="会话密钥" />
...
}))(Form.create()(WechatLoginInfoUpdateForm))
