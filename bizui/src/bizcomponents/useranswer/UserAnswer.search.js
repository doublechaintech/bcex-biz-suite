...
const showAssociateDialog = (targetComponent) => {
  ...
  const { QuestionAssociateForm } = GlobalComponents
  const { ExamAssociateForm } = GlobalComponents
  ...
  <div>
	...
	visible={currentAssociateModal==='question'} 
	data={{userAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'question')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'question')}/> <ExamAssociateForm 
	visible={currentAssociateModal==='exam'} 
	data={{userAnswerList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'exam')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'exam')}/> 
...
class UserAnswerSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {UserAnswerTable} = GlobalComponents;
    const {UserAnswerSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <UserAnswerSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,UserAnswerModalTable,"userAnswerIds")}
...
export default Form.create()(UserAnswerSearch)
