...
const imageList =(formAction)=>{return [
	 ]}

const internalImageListOf = (formAction) =>defaultImageListOf(formAction,imageList)

const optionList =(formAction)=>{return [ 
	]}

const buildTransferModal = defaultBuildTransferModal
const showTransferModel = defaultShowTransferModel
const internalRenderSubjectList = defaultRenderSubjectList
const internalSettingListOf = (formAction) =>defaultSettingListOf(formAction, optionList)
const internalLargeTextOf = (formAction) =>{
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/formAction/${targetComponent.props.formAction.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/formAction/${targetComponent.props.formAction.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
class FormActionDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    if(!this.props.formAction.class){
      return null
    }
...
}))(Form.create()(FormActionDashboard))
