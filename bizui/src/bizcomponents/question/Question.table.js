...
import styles from './Question.table.less'
...
class QuestionTable extends PureComponent {
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = QuestionBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/question/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
