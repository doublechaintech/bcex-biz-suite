...
import styles from './AnswerQuestion.updateform.less'
...
class AnswerQuestionUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const answerQuestionId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, answerQuestionId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateAnswerQuestion`,
          payload: {
            id: owner.id,
            type: 'answerQuestion',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAnswerQuestionUpdateRow`,
        payload: {
          id: owner.id,
          type: 'answerQuestion',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}回答问题: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.answer} {...formItemLayout}>
                  {getFieldDecorator('answer', {
                    initialValue: selectedRow.answer,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <Input size="large" placeholder="回答" />
...
}))(Form.create()(AnswerQuestionUpdateForm))
