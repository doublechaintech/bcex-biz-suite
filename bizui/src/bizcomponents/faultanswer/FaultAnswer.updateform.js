...
import styles from './FaultAnswer.updateform.less'
...
class FaultAnswerUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const faultAnswerId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, faultAnswerId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateFaultAnswer`,
          payload: {
            id: owner.id,
            type: 'faultAnswer',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextFaultAnswerUpdateRow`,
        payload: {
          id: owner.id,
          type: 'faultAnswer',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}错误的答案: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.yourAnswer} {...formItemLayout}>
                  {getFieldDecorator('yourAnswer', {
                    initialValue: selectedRow.yourAnswer,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(FaultAnswerUpdateForm))
