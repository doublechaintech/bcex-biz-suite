...
import styles from './StartExam.table.less'
...
class StartExamTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = StartExamBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/startExam/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
