...
import styles from './Platform.table.less'
...
class PlatformTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = PlatformBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/platform/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
