...
export default {
  ...
  effects: {
    ...
    *addStartExam({ payload }, { call, put }) {
      const userContext = null
      const {ChangeRequestService} = GlobalComponents;

      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ChangeRequestService.addStartExam, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/changeRequest/${id}/list/StartExamList/开始考试+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateStartExam({ payload }, { call, put }) {
      const userContext = null
      const {ChangeRequestService} = GlobalComponents;      
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ChangeRequestService.updateStartExam, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/changeRequest/${id}/list/StartExamList/开始考试列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    *gotoNextStartExamUpdateRow({ payload }, { call, put }) {
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      const newPlayload = { ...payload, selectedRows, currentUpdateIndex }
      yield put({ type: 'updateState', payload: newPlayload })
    },
    *removeStartExamList({ payload }, { call, put }) {
     const userContext = null
      const {ChangeRequestService} = GlobalComponents; 
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ChangeRequestService.removeStartExamList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *addAnswerQuestion({ payload }, { call, put }) {
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ChangeRequestService.addAnswerQuestion, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/changeRequest/${id}/list/AnswerQuestionList/回答问题+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateAnswerQuestion({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ChangeRequestService.updateAnswerQuestion, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/changeRequest/${id}/list/AnswerQuestionList/回答问题列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeAnswerQuestionList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ChangeRequestService.removeAnswerQuestionList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
